...
int main(int argc, char *argv[])        
{
	...
	if (irank == master) {
		...
		printf("Master's partial sum is : %d\n", summ);

		//allocate a sums array to store all the results gathered from the children
		int* sums = (int *)malloc(sizeof(int)*nprocs);
		MPI_Gather(&summ, 1, MPI_INT, sums, 1, MPI_INT, 0,MPI_COMM_WORLD);
		//printf("Master has all the partial sums.\n");
		...
		MPI_Gather(&summ, 1, MPI_INT, NULL, 1, MPI_INT, master,MPI_COMM_WORLD);

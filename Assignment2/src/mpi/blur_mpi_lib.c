...
void pgm_blur_halo(  pgm* input_img , const kernel_t* k,  const uint8_t* halos) {
	...
	*/
	register size_t bound_l = halos[0]*k->halfsize[0];
	register size_t bound_u = halos[1]*k->halfsize[1];
	register size_t bound_r = xdim - halos[2]*k->halfsize[0];
	register size_t bound_d = ydim - halos[3]*k->halfsize[1];

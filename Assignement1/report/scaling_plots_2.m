...
coeffs = [serial_coef(2)];

params = zeros(2,1);

x_data = transpose(cores);

y_data =  [ int_scaling_strong08(:,1) ; int_scaling_strong09(:,1); int_scaling_strong10(:,1); int_scaling_strong11(:,1) ];


fun = @(params,x) [th_scaling_ov1(x,params,coeffs,1e08);th_scaling_ov1(x,params,coeffs,1e09);th_scaling_ov1(x,params,coeffs,1e10);th_scaling_ov1(x,params,coeffs,1e11)];
...
plot(cores,th_scaling_ov1(cores,params,coeffs,1e08) ,'LineWidth',1.8);
plot(cores,th_scaling_ov1(cores,params,coeffs,1e09) ,'LineWidth',1.8);
plot(cores,th_scaling_ov1(cores,params,coeffs,1e10) ,'LineWidth',1.8 );
plot(cores,th_scaling_ov1(cores,params,coeffs,1e11) ,'LineWidth',1.8);
...
function [scaling_out] = th_scaling_ov1(cores,params,coef,N)
   ...
   scaling_out = (N*p)./( (p*N + (s1*N/1e08).*(cores - 1) ).*cores_inv +  k1.*(cores - 1) );
    %scaling_out=transpose(scaling_out);
